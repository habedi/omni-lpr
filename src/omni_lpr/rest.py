...
async def invoke_tool(request: Request) -> JSONResponse:
    ...
    if tool_name not in tool_registry._tools:
        error = ErrorResponse(error={"code": "NOT_FOUND", "message": f"Tool '{tool_name}' not found."})
    ...
    try:
        ...
        error = ErrorResponse(error={"code": "INVALID_JSON", "message": "Request body is not valid JSON."})
        return JSONResponse(error.model_dump(), status_code=400)
    ...
    try:
        ...
        error = ErrorResponse(error={"code": "INTERNAL_SERVER_ERROR", "message": "An internal server error occurred."})
